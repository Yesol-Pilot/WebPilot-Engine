...
import { KeyboardControls, useKeyboardControls, useAnimations } from '@react-three/drei';
    ...
    useFrame(() => {
        ...
        if (!isFreeCamera) {
            ...
            for (const hit of hits) {
                if (hit.distance < 0.5) continue; // [FIX] 너무 가까운 충돌 무시 (0.3→0.5)
                if (hit.object.userData?.isPlayer || hit.object.parent?.userData?.isPlayer) continue;

                // [Fix] 실내 환경 카메라 충돌 개선
                // 벽/가구 앞에서 더 일찍 멈추도록 마진 증가
                if (hit.distance < distToCam) {
                    actualDistance = hit.distance - 1.0; // [FIX] 벽에서 1.0m 떨어지기 (0.5→1.0)
                    break;
                }
            }

            // [Fix] 최소 거리 증가 - 대형 오브젝트 근처에서도 시야 확보
            actualDistance = Math.max(1.5, actualDistance); // [FIX] 0.3→1.5

            // [Fix] 부드러운 카메라 전환 (lerp)
            const targetPos = pivot.clone().add(dirToCam.multiplyScalar(actualDistance));
            camera.position.lerp(targetPos, 0.15); // 부드러운 이동
            camera.lookAt(pivot);
        }
...
function CharacterModel({ direction, grounded }: { direction: THREE.Vector3, grounded: React.MutableRefObject<boolean> }) {
    ...
    const { scene, animations } = useSafeGLTF('https://raw.githubusercontent.com/mrdoob/three.js/master/examples/models/gltf/RobotExpressive/RobotExpressive.glb');
...
useSafeGLTF.preload('https://raw.githubusercontent.com/mrdoob/three.js/master/examples/models/gltf/RobotExpressive/RobotExpressive.glb');
