                    ...
                    return (
                        <Suspense key={semanticName} fallback={<Placeholder position={obj.position || [0, 1, 0]} />}>
                            <SceneObjectWrapper
                                ...
                                initialPosition={obj.position || ((obj as any).transform?.position) || [0, 0, 0]}
                                initialScale={obj.scale || ((obj as any).transform?.scale) || [1, 1, 1]}
                                initialRotation={obj.rotation || ((obj as any).transform?.rotation) || [0, 0, 0]}
                                spatialDesc={obj.description || ''}
                                modelUrl={(obj as any).modelUrl} // [FIX] Critical: Forward modelUrl (Type cast needed)
                            />
                        </Suspense>
                    );
                })}
