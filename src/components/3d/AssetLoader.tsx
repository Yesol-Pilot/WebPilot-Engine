...
interface AssetLoaderProps {
    ...
    id?: string;
    isNpc?: boolean;
    modelUrl?: string; // [FIX] Allow direct URL injection
}
...
const FBXModel = ({ url, position, ...props }: any) => {
    ...
    const fbx = useLoader(require('three/examples/jsm/loaders/FBXLoader').FBXLoader as any, url, (loader: any) => {
...
const GLTFModel = ({ url, position, ...props }: { url: string, position: [number, number, number], [key: string]: any }) => {
    ...
    const gltf = useSafeGLTF(url) as any;
...
export const AssetLoader = React.memo(({
    description, position, rotation, scale, onInteract, onSelect, withPhysics = true, id, type, modelUrl: propModelUrl
}: AssetLoaderProps) => {
