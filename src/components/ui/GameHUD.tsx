...
export const GameHUD = ({ className, cameraMode = 'follow', onToggleCamera }: GameHUDProps) => {
    const [showControls, setShowControls] = React.useState(true);
    const [isInventoryOpen, setIsInventoryOpen] = React.useState(false);
    const [isScenarioSelectorOpen, setIsScenarioSelectorOpen] = React.useState(false);
    ...
    return (
        <div className={`fixed inset-0 pointer-events-none z-50 ${className}`}>
            ...
            {isScenarioSelectorOpen && <ScenarioSelector onClose={() => setIsScenarioSelectorOpen(false)} />}
            ...
            <div className="absolute bottom-8 right-8 hidden md:flex flex-col items-end gap-2 pointer-events-auto">
                ...
                <button
                    ...
                    className={`bg-black/40 hover:bg-black/60 p-2 rounded-full backdrop-blur transition-all border border-white/10 ${cameraMode === 'free' ? 'text-green-400 border-green-400/50' : 'text-white/50 hover:text-white'}`}
                    title={`Camera Mode: ${cameraMode === 'free' ? 'Free Fly' : 'Third Person'} (C)`}
                >
                    {cameraMode === 'free' ? '🦅' : '👤'}
                </button>
