...
const MOCK_SCENARIO: Scenario = DEFAULT_SCENARIO;
...
interface GameLogicProps {
    ...
    onModeToggle: () => void;
    isEditMode: boolean;
    showDevUI: boolean;  // 개발자 UI 표시 여부
}

function GameLogic({ scenario, setScenario, onModeToggle, isEditMode, showDevUI }: GameLogicProps) {
        ...
        <>
            <div className="absolute top-0 left-0 w-full p-4 pointer-events-none flex justify-between items-start z-50">
                <div className="bg-black/50 text-white p-4 rounded backdrop-blur-sm pointer-events-auto mt-14">
                    ...
                    <p className="opacity-80 text-sm max-w-md mt-2">{scenario.narrative?.intro}</p>
                </div>
...
interface GameClientProps {
    ...
    mode?: 'editor' | 'game' | 'immersive';  // UI 모드 초기값
}

export default function GameClient({ initialScenario, mode: initialMode }: GameClientProps) {
    ...
    const {
        ...
        inventory, flags,
        uiMode, showDevUI, setUIMode, toggleDevUI,  // UI 모드 추가
    ...
    // 첫 렌더링에서도 initialMode 우선 적용 (useEffect 실행 전 대응)
    const effectiveUIMode = initialMode || uiMode;
        ...
        if (typeof window !== 'undefined') {
            ...
            if (scenarioId) {
                ...
                if (scenarioId === 'sorting_ceremony_01' || scenarioId === 'sorting_hat') {
                    targetScenario = LEGACY_SCENARIOS.SortingCeremony;
                } else {
                    // LEGACY_SCENARIOS에서 호그와트 기숙사 검색 (Gryffindor 등)
                    const legacy = LEGACY_SCENARIOS as Record<string, Scenario>;
                    targetScenario = Object.values(legacy).find(s => s.id === scenarioId);
                }
                if (targetScenario) {
    ...
    }, [toggleDevUI, isInputFocused]);

    // 소팅 시나리오 진입 시 immersive 모드
    useEffect(() => {
        if (scenario?.id?.includes('sorting') || initialScenario?.id?.includes('sorting')) {
            setUIMode('immersive');
        }
    }, [scenario?.id, initialScenario?.id, setUIMode]);
    ...
    return (
        <div className="relative w-full h-full">
            <GameCanvas
                ...
                showDebugHUD={showDevUI} // [Fix] 개발자 모드일 때만 DEBUG HUD 표시
            />

            {/* GameLogic - immersive 모드에서는 숨김 */}
            {scenario && effectiveUIMode !== 'immersive' && (
                <GameLogic
                    key={reloadKey}
            ...
            {/* Game UI - game 모드에서만 표시 */}
            {effectiveUIMode === 'game' && <GameHUD />}
            {effectiveUIMode === 'game' && <InventoryHUD />}
            <AudioManager />
            {keypadTarget && (
                <Keypad
                    targetCode="1234"
            ...
            {effectiveUIMode === 'game' && <QuestTracker />}
            {effectiveUIMode === 'game' && <QuestJournal />}

            {/* Crosshair - game 모드에서만 */}
            {effectiveUIMode === 'game' && (
            ...
            {effectiveUIMode === 'game' && hoverText && (
            ...
            {/* Camera Toggle - immersive 모드에서는 숨김 */}
            {effectiveUIMode !== 'immersive' && (
