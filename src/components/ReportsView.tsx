...
import { Report, GitChangelog, GitPush } from '@/lib/reports';
import GeneratedCover from './GeneratedCover';
import { motion, AnimatePresence } from 'framer-motion';

import GitChangelogView from './GitChangelogView';
...
export default function ReportsView({
    ...
    dailyReports: Report[],
    docReports: Report[],
    deploymentReports: Report[],
    gitChangelog?: GitChangelog | null
}) {
    const [activeTab, setActiveTab] = useState<'docs' | 'daily' | 'git'>('docs');
        ...
        <div className="container mx-auto px-4 max-w-6xl -mt-12 relative z-10">
            ...
            <div className="flex justify-center mb-12">
                <div className="bg-white dark:bg-zinc-900 p-1.5 rounded-full shadow-lg border border-gray-100 dark:border-zinc-800 inline-flex flex-wrap justify-center gap-1">
                    ...
                    <button
                        ...
                        className={`px-6 py-3 rounded-full text-sm font-bold transition-all duration-300 ${activeTab === 'git'
            ...
            <AnimatePresence mode="wait">
                {activeTab === 'git' ? (
                    ...
                    >
                        {gitChangelog && gitChangelog.pushHistory.length > 0 ? (
                            <GitChangelogView changelog={gitChangelog} />
                        ) : (
                            <div className="text-center py-20">
                                <p className="text-gray-500 dark:text-gray-400 text-lg">No Git history found.</p>
                            </div>
                        )}
                    </motion.div>
                ) : (
                    ...
                    >
                        {activeTab === 'docs' && docReports.map((report) => (
                            <ReportCard key={report.slug} report={report} />
                        ))}
                        {activeTab === 'daily' && dailyReports.map((report) => (
                            <ReportCard key={report.slug} report={report} />
                        ))}
                    </motion.div>
                )}
            ...
            {/* Empty State for Docs/Daily */}
            {activeTab !== 'git' &&
                ((activeTab === 'docs' && docReports.length === 0) ||
                    (activeTab === 'daily' && dailyReports.length === 0)) && (
                    <div className="text-center py-20">
