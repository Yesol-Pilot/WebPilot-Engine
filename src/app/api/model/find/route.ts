...
import { generate3DAsset } from '../generate/route'; // Internal Import
...
async function getAllPrompts(): Promise<{ prompt: string; filePath: string; source: string }[]> {
    ...
    try {
        ...
        dbAssets.forEach(a => {
            if (a.prompt) {
                allPrompts.push({ prompt: a.prompt, filePath: a.filePath, source: 'database' });
            }
        });
    } catch (dbError) {
...
export async function POST(req: NextRequest) {
    try {
        ...
        // 4. 캐시에 없으면 새로 생성 (Tripo API 호출)
        console.log(`[Model Find] Not found: "${prompt.slice(0, 50)}...". Triggering Tripo generation...`);

        try {
            // Internal Function Call (No HTTP round-trip)
            const generated = await generate3DAsset(prompt) as { success: boolean; modelUrl?: string; taskId?: string };
            ...
            console.error(`[Model Find] Auto-generation failed for "${prompt}":`, genError);
            // If generation fails (e.g. Rate Limit), fall back to null so the client shows a red box
        }
