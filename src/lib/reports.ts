...
// Git Changelog 데이터 로딩
export function getGitChangelog(): GitChangelog | null {
    const changelogPath = path.join(process.cwd(), '.agent/data/git_changelog.json');

    if (!fs.existsSync(changelogPath)) {
        console.warn('[reports] git_changelog.json not found');
        return null;
    }

    try {
        const content = fs.readFileSync(changelogPath, 'utf8');
        return JSON.parse(content) as GitChangelog;
    } catch (error) {
        console.error('[reports] Failed to parse git_changelog.json:', error);
        return null;
    }
}
